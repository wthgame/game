...
export class CharacterController {
	...
	public getMaybeRoot(): Maybe<BasePart> {
		return this.getMaybeCharacter()?.FindFirstChild("HumanoidRootPart");
	}

	public async getRoot(): Promise<BasePart> {
		return (await this.getCharacter()).WaitForChild("HumanoidRootPart");
	}

	public getMaybeHumanoid(): Maybe<Humanoid> {
		return this.getMaybeCharacter()?.FindFirstChild("Humanoid");
	}

	public async getHumanoid(): Promise<Humanoid> {
		return (await this.getCharacter()).WaitForChild("Humanoid");
	}
}
